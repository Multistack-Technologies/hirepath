...
export default function CandidatesList() {
  ...
  const fetchCandidates = async (url: string = '/applications/recruiter/candidates/') => {
    setIsLoading(true);
    setError(null);
  ...
  useEffect(() => {
    ...
    // Status filter
    if (statusFilter !== 'ALL') {
      filtered = filtered.filter(candidate => candidate.application_status === statusFilter);
    }
    ...
    if (scoreFilter !== 'ALL') {
      switch (scoreFilter) {
        case 'EXCELLENT':
          filtered = filtered.filter(candidate => candidate.match_score >= 80);
          break;
        case 'GOOD':
          filtered = filtered.filter(candidate => candidate.match_score >= 60 && candidate.match_score < 80);
          break;
        case 'FAIR':
          filtered = filtered.filter(candidate => candidate.match_score >= 40 && candidate.match_score < 60);
          break;
        case 'POOR':
    ...
    // Job filter
    if (jobFilter !== 'ALL') {
      filtered = filtered.filter(candidate => candidate.job_title === jobFilter);
    }
  ...
  const uniqueJobTitles = [...new Set(candidates.map(candidate => candidate.job_title))];

  // Calculate statistics
  const stats = {
    total: pagination.count,
    pending: candidates.filter(c => c.application_status === 'PENDING').length,
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="bg-white rounded-2xl border border-gray-200 p-6 mb-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-4">
            ...
            {(searchQuery || statusFilter !== 'ALL' || scoreFilter !== 'ALL' || jobFilter !== 'ALL') && (
              <div className="flex items-center justify-between">
                <span className="text-sm text-gray-600">
                  Showing {filteredCandidates.length} of {candidates.length} loaded candidates
                  {pagination.count > candidates.length && ` (${pagination.count} total)`}
                </span>
                <button
                  onClick={() => {
                    setSearchQuery('');
            ...
            {pagination.next && (
              <div className="mt-6 text-center">
                ...
                <p className="text-sm text-gray-500 mt-2">
                  Showing {candidates.length} of {pagination.count} total candidates
                </p>
              </div>
            )}
          </>
